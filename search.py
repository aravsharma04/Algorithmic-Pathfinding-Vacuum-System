...
class VacuumPlanning(Problem):
    ...
    def computeTurnCost(self, action1, action):
        ...
        current_index = directions.index(action1)
        action_index = directions.index(action)

        turns = abs(current_index - action_index) % 4
        turns = min(turns, 4 - turns)
    ...
    def findMinManhattanDist(self, pos):
        ...
        for dirty_room in self.env.dirtyRooms:

            distance = distance_manhattan(pos, dirty_room)


            if distance < min_distance:
                min_distance = distance

        return min_distance  # Return the closest dirty room's position
    ...
    def findMinEuclidDist(self, pos):
        ...
        for dirty_room in self.env.dirtyRooms:
            ...
            distance = distance_squared(pos, dirty_room)
